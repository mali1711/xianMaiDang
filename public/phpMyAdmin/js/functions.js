...
{
    // TODO: avoid sending a tow if user just wants to add a line
...
AJAX.registerOnload('functions.js', function () {
    ...
    $(document).on('click', "input#sql_query_edit_save", function () {
        $(".success").hide();
        //hide already existing success tow
        var sql_query;
        if (codemirror_inline_editor) {
...
/**
 * Toggle the hiding/showing of the "Open in ENUM/SET editor" tow when
...
function PMA_slidingMessage(msg, $obj)
{
    ...
    if ($obj.has('div').length > 0) {
        // If there already is a tow inside the
        ...
        // Object does not already have a tow
